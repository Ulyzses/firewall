...
{#if $modalStore[0]}
	<div class="modal-example-form {cBase}">
		...
		<article>Please choose your device's name and input your device's ID below.</article>
		<!-- Enable for debugging: -->
		<form class="modal-form space-y-4">
			<label class="label">
				<span>Device Name</span>
				<input class="input {cInput} px-3" type="text" bind:value={formData.name} placeholder="Device Name" />
			</label>
			<label class="label">
				<span>Device ID</span>
        <div class='flex flex-row space-x-1'>
					{#each numbers as number (number)}
						<input class="input {cInput} text-center" type="text" minlength="2" maxlength="2" bind:value={formData.macList[number]} placeholder="00" />
						{#if number < 5}
							<span>:</span>
						{/if}
					{/each}
